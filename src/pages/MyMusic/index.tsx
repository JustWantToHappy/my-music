...
const Index = () => {
  ...
  const handleMenu = (e: any) => {
    if (e.key !== 'add') {
      ...
      navigate(`mysonglist?id=${e.key}`, {
        state: { data: songlist && songlist[index], mylove },
      })
    } else {
  ...
  const getData = () => {
    ;(async () => {
      ...
      if (localStorage.getItem('hasLogin') === 'true') {
        ...
        setList(playlist)
        let arr = []
        arr.push(getItem(<span>新建歌单</span>, 'add', <PlusCircleOutlined />))
        let brr: Array<MenuItem> = []
        playlist &&
          playlist.forEach((list: User.songList) => {
            brr.push(
              getItem(
                list.name,
                list.id,
        ...
        if (playlist.length > 0) {
          navigate(`/mymc/mysonglist?id=${playlist[1].id}`, {
            state: { data: playlist[1], mylove: false },
          })
        } else {
          navigate(`mysonglist?id=${playlist[0].id}`, {
            state: { data: playlist[0], mylove: true },
          })
        }
      }
    })()
