    ...
    <>
      ...
      {pathname === '/home' && (
        <div className={styles.container}>
          <Carousel style={{ height: '100%', width: `100vw` }} autoplay>
            {arr?.map((mv) => {
              return (
                <img src={mv.cover} className={styles.carousel} alt={mv.name} />
              )
            })}
          </Carousel>
        </div>
      )}
      {hasLogin && pathname === '/home' && (
        <div className={styles['my-recommon']}>
          <MyRecommend />
        </div>
      )}
      {/* 精选歌单*/}
