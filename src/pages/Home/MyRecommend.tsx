      ...
      <div>
        <div className={styles['songs-recommend']}>
          <div onClick={playRecommendSongs}>
            <h3>星期{transNumber(new Date().getDay())}</h3>
            <span>{new Date().getDate()}</span>
          </div>
          <footer onClick={playRecommendSongs}>每日歌曲推荐</footer>
        </div>
        {songList.slice(0, 9).map((songlist: User.recommendList) => {
          return (
            <div
              ...
              onClick={() => {
                navigate(`/playlist/${songlist.id}`)
              }}
            >
              <div
                style={{
                  backgroundImage: `url(${songlist.picUrl})`,
              ...
              >
                ...
                >
                  ...
                  <PlayCircleOutlined
                    onClick={(e) => {
                      playRecommendSongList(e, songlist.id)
                    }}
                  />
                </figcaption>
              </div>
